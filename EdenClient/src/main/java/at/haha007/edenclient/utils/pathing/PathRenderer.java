...
public class PathRenderer {
    public static void renderPath(Path path, float deltaTime) {
        ...
        for (int i = 0; i < nearest.block(); i++) {
            ...
            linesFrom.add(new Pair<>(block.startPos().add(0,.01,0), block.endPos().add(0,.01,0)));
        }
        linesFrom.add(new Pair<>(currentBlock.startPos().add(0,.01,0), currentBlock.pointAlongBlock(progress).add(0,.01,0)));

        List<Pair<Vec3, Vec3>> linesTo = new ArrayList<>();
        for (int i = nearest.block() + 1; i < positions.size(); i++) {
            PathBlock block = path.getBlock(i);
            if (block == null) continue;
            linesTo.add(new Pair<>(block.startPos().add(0,.01,0), block.endPos().add(0,.01,0)));
        }
        linesTo.add(new Pair<>(currentBlock.pointAlongBlock(progress).add(0,.01,0), currentBlock.endPos().add(0,.01,0)));
