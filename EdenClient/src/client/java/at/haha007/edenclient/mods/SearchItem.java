        ...
        for (Item item : BuiltInRegistries.ITEM) {
            cmd.then(CommandManager.literal(BuiltInRegistries.ITEM.getKey(item).getPath()).executes(c -> {
                ...
                nearest.ifPresentOrElse(pos -> {
                    ...
                    PlayerUtils.sendModMessage("Nearest container with " + item.getDefaultInstance().getDisplayName().getString() + " found");
                }, () -> PlayerUtils.sendModMessage("No containers with " + item.getDefaultInstance().getDisplayName().getString() + " found"));
                return 1;
            }));
        }
